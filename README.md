👋 Welcome to My GitHub Profile
Hi there, I'm Sanny Sharma 👨‍💻
Frontend Developer | Web Development Enthusiast | Java DSA Learner
Typing SVG

🎯 About Me
I'm a BTech CSE Student from India with a passion for frontend development and building beautiful, responsive web applications. I'm currently focused on mastering Web Development and Data Structures & Algorithms in Java.

🎓 Currently studying Computer Science Engineering
💻 Specialized in Frontend Development (HTML, CSS, JavaScript, React)
📚 Learning Java DSA and Full Stack Development
🚀 Building projects to strengthen my portfolio
🎨 Passionate about creating user-friendly interfaces
🛠️ Tech Stack
HTML5
CSS3
JavaScript
React
Tailwind CSS
Java
Git
VS Code

📊 GitHub Statistics
GitHub Streak

Top Languages

💼 Featured Projects
🎯 Modern Portfolio Website
A responsive personal portfolio website showcasing my projects and skills.

Tech: HTML, CSS, JavaScript
Features: Responsive design, Smooth animations, Project showcase
Live Demo: [Visit Site]
🛒 Amazon Clone
Responsive Amazon homepage clone with modern UI.

Tech: HTML, CSS
Features: Grid layout, Responsive design, Product cards
🤖 CodeMentor AI
AI-powered coding assistant using Google Gemini API.

Tech: React, Tailwind CSS, Gemini API
Features: Real-time coding help, Clean UI, API integration
⏰ Digital Clock
Interactive digital clock displaying current date and time.

Tech: HTML, CSS, JavaScript
Features: Real-time updates, Responsive design
📈 What I'm Currently Working On
🔨 Building responsive React applications
📖 Mastering Data Structures & Algorithms
🎓 Completing web development projects
💪 Strengthening my problem-solving skills
🎓 Learning Roadmap
Frontend Development
    ✅ HTML5 & CSS3
    ✅ JavaScript Fundamentals
    ✅ Responsive Design
    🔄 React.js (In Progress)
    ⏳ State Management
    ⏳ Advanced CSS

Backend Development
    ⏳ Node.js & Express
    ⏳ Databases (MongoDB, MySQL)

Data Structures & Algorithms
    🔄 Java (In Progress)
    ⏳ Problem Solving
📞 Connect With Me
LinkedIn
GitHub
Portfolio
Gmail
Twitter

🏆 GitHub Achievements
Commits Badge
Repositories
Followers

💡 Quick Facts
🎯 Goal: Become a Full Stack Developer
⚡ Specialty: Frontend Development & Responsive Design
📱 Passionate About: Creating user-friendly web experiences
🌱 Currently Learning: React.js & Advanced JavaScript
💬 Ask Me About: HTML, CSS, JavaScript, Responsive Design
📚 Resources I Recommend
MDN Web Docs - Comprehensive web development documentation
CSS Tricks - Advanced CSS techniques
JavaScript.info - Modern JavaScript guide
React Documentation - Official React docs
LeetCode - DSA practice
🎉 Fun Facts
💻 I love building things on the web
🎨 Design and user experience matter to me
📊 Always analyzing and improving my code
🚀 Excited about new technologies
👥 Open to collaboration and learning from others
Let's Connect & Collaborate! 🤝
Feel free to reach out if you want to discuss web development, collaborate on projects, or just chat about code!

Profile Views

Last updated: 2026 | Keep learning, keep growing! 🚀
